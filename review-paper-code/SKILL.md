...
## Phase 3: Complete 2 Review Passes

Complete the 2 review passes below. If the environment supports delegation and the user explicitly asks for parallel agent work, you may delegate them. Otherwise, perform both passes yourself in sequence and keep the output structure unchanged.

Each pass must produce a compact, high-signal output. Do not ask for exhaustive per-file prose on every file unless the project is very small.
...
After both review passes are complete, synthesize the results yourself.

Do not add another critic pass by default. Instead:
...
If the repo is unusually complex and a second-pass critic is truly necessary, you may run one additional critic pass. Otherwise, keep the workflow lean.
