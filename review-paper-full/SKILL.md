...
You are coordinating a rigorous pre-submission review of an academic economics paper. Work through 6 specialized review passes and consolidate their findings into a structured report.
...
Store the resolved target journal as `TARGET_JOURNAL` for use in Pass 6 and the report header.
...
**If zero table files are found**, warn the user: "No table .tex files were found in standard locations. Tables may be stored in an `output/` or non-standard directory. The tables and figures documentation pass will only be able to check table captions and cross-references from the main .tex files."

## Phase 2: Complete 6 Review Passes

Complete the 6 review passes below. If the environment supports delegation and the user explicitly asks for parallel agent work, you may delegate individual passes. Otherwise, perform them yourself in sequence and preserve the per-pass output structure. Each pass should read the relevant paper files independently enough to stand on its own. When you reach Pass 6, add the following line at the top of that evaluation: "The target journal is [resolved value of TARGET_JOURNAL]." Do not rewrite the journal-dependent conditional logic inside the template.
...
**Before consolidating**, check for missing or malformed pass outputs: if any review pass returned no usable output, insert a placeholder section in the report (e.g., "## 4. Mathematics, Equations & Notation — Review pass did not return output") and include it in the final user-facing summary.

After all available review pass results are collected, consolidate them into a single structured report.
